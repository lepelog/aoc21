...
impl BingoCard {
    ...
    pub fn unmarked_sum(&self) -> usize {
        self.nums
            .iter()
            .zip(self.crossed_out.iter())
            .filter_map(|(num, cro)| if *cro { None } else { Some(*num as usize) })
            .sum()
    }
}
...
pub fn read_puzzle() -> (Vec<u8>, Vec<BingoCard>) {
    ...
    loop {
        let mut current_card = BingoCard {
            nums: [0; 25],
            crossed_out: [false; 25],
        };
        let row1 = line_itr
            .next()
            .unwrap()
